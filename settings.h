...
inline void initImageEditor(SDL_Renderer* renderer, ImageEditorState* editor, int canvasW, int canvasH) {
    ...
    editor->lastX      = -1;
    editor->lastY      = -1;
    editor->penColor.r=0;editor->penColor.g=0;editor->penColor.b=0;editor->penColor.a=255;
...
static const int CHAR_PANEL_W    = 110;
static const int CHAR_PANEL_X    = 0;
static const int CHAR_PANEL_Y    = 40;

static const int SETT_PANEL_W    = 200;
static const int SETT_PANEL_X    = WINDOW_W - SETT_PANEL_W;
static const int SETT_PANEL_Y    = 40;

static const int TOOLBAR_H       = 40;

static const int STAGE_X         = CHAR_PANEL_W;
static const int STAGE_Y         = TOOLBAR_H;
static const int STAGE_W         = WINDOW_W - CHAR_PANEL_W - SETT_PANEL_W;
static const int STAGE_H         = WINDOW_H - TOOLBAR_H;

static const SDL_Rect BG_BTN_RECT = { 10, 5, 120, 30 };

static const int EDITOR_CANVAS_W = 760;
static const int EDITOR_CANVAS_H = 480;
static const int EDITOR_CANVAS_X = STAGE_X + (STAGE_W - EDITOR_CANVAS_W) / 2;
static const int EDITOR_CANVAS_Y = STAGE_Y + 20;
static const SDL_Rect EDITOR_CANVAS_RECT = {
        EDITOR_CANVAS_X, EDITOR_CANVAS_Y, EDITOR_CANVAS_W, EDITOR_CANVAS_H
};

static bool simpleTextInputDialog(SDL_Renderer* renderer,
                                  TTF_Font*     font,
                                  const char*   prompt,
                                  char*         outBuf,
                                  int           bufLen) {
    SDL_StartTextInput();
    char text[256] = {0};
    strncpy(text, outBuf, sizeof(text) - 1);
    ...
    while (!done) {
        ...
        while (SDL_PollEvent(&e)) {
            ...
            if (e.type == SDL_TEXTINPUT) {
                strncat(text, e.text.text, sizeof(text) - strlen(text) - 1);
            }
        }
        ...
        SDL_Surface* ps = TTF_RenderText_Blended(font, prompt, white);
        if (ps) {
        ...
        SDL_Surface* ts = TTF_RenderText_Blended(font, display, cyan);
        if (ts) {
        ...
        SDL_Color ty={150, 150, 150, 255};
        SDL_Surface* hs = TTF_RenderText_Blended(font, "Enter = confirm    Esc = cancel", ty);
        if (hs) {
...
static void renderToolbar(SDL_Renderer* renderer, TTF_Font* font,
                          BackgroundMenuState* bgMenu, BackgroundState* bgState) {
    SDL_SetRenderDrawColor(renderer, 20, 20, 35, 255);
    SDL_Rect toolbar = {0, 0, WINDOW_W, TOOLBAR_H};
    SDL_RenderFillRect(renderer, &toolbar);

    renderBackgroundMenu(renderer, font, bgMenu, bgState,
                         const_cast<SDL_Rect&>(BG_BTN_RECT));

    SDL_Color title_col = {180, 180, 255, 255};
    SDL_Surface* ts = TTF_RenderText_Blended(font, "Scratch-Like Editor", title_col);
    if (ts) {
        SDL_Texture* tt = SDL_CreateTextureFromSurface(renderer, ts);
        SDL_Rect tr = { WINDOW_W - ts->w - 10, (TOOLBAR_H - ts->h) / 2, ts->w, ts->h };
        SDL_RenderCopy(renderer, tt, NULL, &tr);
        SDL_DestroyTexture(tt);
        SDL_FreeSurface(ts);
    }
